...
В `docker-compose.yml` по умолчанию используются порты **8080** и **8443** (чтобы не конфликтовать с уже занятыми 80/443 на сервере).

- **HTTP:** http://193.42.127.176:8080  
...
Если порты 80 и 443 на сервере свободны и хотите без номера порта — в `docker-compose.yml` замените `8080:80` и `8443:443` на `80:80` и `443:443`, затем откройте фаервол для 80/443.

Убедитесь, что на сервере открыты порты **8080** и **8443** (фаервол / security group в облаке).
